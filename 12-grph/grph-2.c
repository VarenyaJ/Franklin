...
/*
Use the standard C library functions to read the input file to replace/avoid the kseq.h library and extract the DNA strings for each sequence in FASTA format
...
Read the input file line by line using the built-in fgets() function and extract the DNA string for each sequence

Create an empty dictionary adj_list to store the adjacency list. Allocate memory for 1000 key-value pairs

Define a function find_overlap() to find the overlap between two DNA sequences. The function takes two sequences and a value of k as input and returns True if there
is a length k suffix of the first sequence that matches a length k prefix of the second sequence. Otherwise, it returns False

Iterate over each pair of DNA sequences using nested loops and check for overlap of length 3 using
the find_overlap() function. If there is an overlap, add the key-value pair to the adjacency list
...
int main(int argc, char** argv) {
    if (argc != 2) {
        fprintf(stderr, "Usage: %s </c/Users/varen/Desktop/My_New_Stuff/no_place_like/Computing/Bioinformatics/Rosalind/12-grph/rosalind_grph.txt>\n", argv[0]);
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        if (line[0] == '>') {
            if (seq_count > 0) {
                ...
                seq_list[seq_count - 1] = malloc(sizeof(char) * (strlen(sequence) + 1));
                strcpy(seq_list[seq_count - 1], sequence);
                memset(sequence, 0, sizeof(char) * 10000);
            }
            seq_count++;
        } else {
    ...
    for (int i = 0; i < seq_count; i++) {
        for (int j = 0; j < seq_count; j++) {
            if (i != j && find_overlap(seq_list[i], seq_list[j], 3)) {
                // if there is an overlap, add key-value pair to adjacency list
...
// ./grph-2.exe /c/Users/.../12-grph/rosalind_grph.txt