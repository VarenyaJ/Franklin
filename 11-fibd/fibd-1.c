...
int getRabbitPairs(int n, int m) {
    int dp[100] = {0};  // Initialize an array to store the number of rabbit pairs for each month, with a maximum size of 100
    ...
    return dp[n-1];     // Return the number of rabbit pairs for the nth month
}

int main() {
    int n = 36;     // Set the number of months
    int m = 9;     // Set the lifespan of the rabbits
